...
from food_auto.batch_cooking import generate_cooking_instructions, generate_grocery_list
    ...
    while True:
        ...
        for recipe in recipe_list:
            if recipe not in selected_recipes:
                choices.append(
                    f"{recipe.name}\tnumber of portions: {recipe.nb_portions}"
                )
...
def main():
    ...
    )  # Fallback to "recipes" if not set

    # Parse command line arguments
    parser = argparse.ArgumentParser(
        description="Generate batch cooking instructions and grocery list"
    )
    parser.add_argument(
        "--path", type=str, default=default_path, help="Path to recipe directory"
    )
